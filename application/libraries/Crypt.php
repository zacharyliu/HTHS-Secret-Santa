...
class Crypt {
    ...
    public function encrypt($message, $key, $pin = null) { // $key may be public or private key, make sure you provide a pin if private
        $rsa = new Crypt_RSA();
        $rsa->loadKey($key);
        if ($pin != null) {
            $rsa->setPassword($pin);
    ...
    public function decrypt($message, $key, $pin = null) { // $key may be public or private key, make sure you provide a pin if private
        $rsa = new Crypt_RSA();
        $rsa->loadKey($key);
        if ($pin != null) {
            $rsa->setPassword($pin);
